...
def fetch_apple_roots(
    ...
    *,
    ...
    """Fetch Apple's trusted root certificates bundle.

    Config options:
      - url: URL to Apple's root certificate bundle (optional, uses default)
      - verify: may include 'sha256', 'ca_file', and 'tls_fingerprint_sha256' for verification

    Default source: Apple's Root Certificate Program via CCADB
    See: https://www.apple.com/certificateauthority/
    Note: Apple roots are available through the Common CA Database (CCADB) in PEM format
    """
    dest_dir.mkdir(parents=True, exist_ok=True)
    out_path = dest_dir / (name if name.endswith(".pem") else f"{name}.pem")

    # Default to CCADB's Apple roots
    # This is the authoritative source for Apple's root program
    bundle_url = (
        url
        or "https://ccadb.my.salesforce-sites.com/apple/IncludedRootsPEMTxt?TrustBitsInclude=ServerAuth"
    )

    parsed = urllib.parse.urlparse(bundle_url)
    if parsed.scheme.lower() != "https":
        raise click.ClickException("Apple roots fetch requires HTTPS URL")
    ...
    try:
        ...
        msg = f"Failed to fetch Apple roots: HTTP Error {e.code}: {e.reason}"
