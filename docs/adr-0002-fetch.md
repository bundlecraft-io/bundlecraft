...
1. Define `fetch:` schema additions in `config/cert_sources/*.yaml`.

1. Update manifest spec to include:

   ```json
   "sources": [{"path": "...", "origin": "...", "sha256": "..."}]
   ```

1. Builder fetch behavior: default to fetching; support `--skip-fetch` for offline/iterative builds (replace older `--prefetch`/`--offline` ideas with simpler semantics).

1. Extend tests to validate fetch behavior and provenance embedding.

1. Update documentation and diagrams (README, CLI reference, pipeline overview).
