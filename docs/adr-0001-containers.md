...
1. **Create Dockerfile (OCI)** with BuildKit optimizations; add CI job to build multi-arch images.

1. **Add signing** (cosign), **SBOM** (syft), **scan** (grype), **provenance** attestation steps to the release workflow.

1. **Publish** images to GHCR; document mirroring to other registries.

1. **Provide a template repo** that:
...
1. **Docs:** Update README/SECURITY/RELEASE to reflect image-based usage; add `podman` examples first, mention `docker` only as compatible.
...
1. Implement GH Actions workflow to build multi-arch, sign, generate SBOM, attach provenance, and publish to GHCR.
1. Draft the template repo (sample config, sample workflow, usage docs).
1. Update README/SECURITY/RELEASE with `podman`-first instructions and digest pinning examples.
1. Announce deprecation of any earlier “install locally” guidance in favor of the containerized engine.
